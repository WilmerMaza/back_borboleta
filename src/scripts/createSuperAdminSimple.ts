...
async function createSuperAdmin() {
  try {
    ...
    const existingUser = await UserModel.findOne({ email: 'borboleta.com@gmail.com' });
    if (existingUser) {
    ...
    // Hash de la contraseña usando bcrypt directamente
    const hashedPassword = await bcrypt.hash('123456', 12);

    // Crear usuario super administrador
    const superAdminData = {
      name: 'Super Administrador',
      email: 'borboleta.com@gmail.com',
    ...
    console.log('   Email: borboleta.com@gmail.com');
    console.log('   Password: 123456');
    console.log('\n📡 Endpoint de login:');
