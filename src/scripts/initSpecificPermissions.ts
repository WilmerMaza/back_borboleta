...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/borboleta";
...
async function initSpecificPermissions() {
  try {
      ...
      if (!existingRole) {
        ...
        const rolePermissions = roleData.permissions
          .map((permissionId: any) => ({
            role_id: role.id,
            permission_id: createdPermissions.find(
              (p) => p._id.toString() === permissionId.toString()
            )?.id,
          }))
        ...
        console.log(
          `   ✅ Rol creado: ${role.name} con ${rolePermissions.length} permisos`
        );
      } else {
    ...
    allRoles.forEach((role) => {
      console.log(
        `   - ${role.name} (${
          role.system_reserve === 1 ? "Sistema" : "Personalizado"
